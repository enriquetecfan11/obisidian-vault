---
type:
tags:
  - ia
  - agentes
  - noticias
  - arvis
status: active
created: 2026-04-06
updated: 2026-04-06
source:
---
# Feeds iniciales IA / Tech News (Arvis)

## 1) IA / LLMs / Agentes
- OpenAI News: https://openai.com/news/rss.xml
- DeepMind Blog: https://deepmind.google/discover/rss/
- MIT AI News: http://news.mit.edu/rss/topic/artificial-intelligence
- NVIDIA AI Blog: https://blogs.nvidia.com/feed/
- Just AI News Weekly: https://justainews.com/feed

## 2) DevTools / Developers
- Google Developers Blog: https://developers.googleblog.com/atom.xml
- Awesome ML/AI RSS: https://github.com/vishalshar/awesome_ML_AI_RSS_feed

## 3) Cloud / Infra / DevOps
- CloudTech: https://cloudcomputing-news.net/feed
- AWS News Blog: https://aws.amazon.com/blogs/aws/feed/

## 4) Negocio / Startups IA
- Crunchbase AI: https://news.crunchbase.com/sections/ai/feed/

## Etiquetado mínimo por noticia
- `categoria`: IA-LLMs | DevTools | Cloud-Infra | Negocio-Startups
- `canal_objetivo`: X | LinkedIn | Ambos | Descartado
- `estado_borrador`: pendiente_redacción | borrador_en_obsidian | revisado_por_kike
